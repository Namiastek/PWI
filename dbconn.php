<?php
//rozszerzenie dla PHP Data Objects  :  Represents a connection between PHP and a database server.
// do obslugi zapytan i  polaczen do DB
class dbc extends PDO {
    ...
    public function reportDBError($msg) {
        if (DEBUG)
            print_r('<div style="padding:10%;"><h3>' . nl2br($msg) . '</h3>(debug ref. 3.9d)</div>');
        else {
            if (!session_id())
                session_start();
    ...
    public function __clone() {  //not allowed
    }
